...
var { adminModel,staffModel,studentModel } = require('../models/user');
...
exports.adminuserLogin = (req, res, next) => {
    ...
    .then(usr => {
        ...
        bcrypt.hashSync(usr.password, salt, function(err, hash) {
  ...
  exports.staffuserLogin = (req, res, next) => {
    let fetchedUser;
  ...
  exports.studentuserLogin = (req, res, next) => {
    let fetchedUser;
  ...
  exports.adminuserLogout = (req, res, next) => {
    let fetchedUser;
    adminModel.findOne({ email: req.body.email })
    .then(usr => {
      if(!usr){
  ...
  exports.staffuserLogout = (req, res, next) => {
    let fetchedUser;
    staffModel.findOne({ email: req.body.email })
    .then(usr => {
      if(!usr){
  ...
  exports.studentuserLogout = (req, res, next) => {
    let fetchedUser;
    studentModel.findOne({ email: req.body.email })
    .then(usr => {
      if(!usr){
  ...
  exports.adminuserCreate = (req, res, next) => {
    req.body.active = true;
    adminModel.create(req.body, (err, r_user) => {
        if(err) {
            console.log(err);
            res.status(503).send(err);
        } else {
...
exports.staffuserCreate = (req, res, next) => {
  req.body.active = true;
  staffModel.create(req.body, (err, r_user) => {
      if(err) {
          console.log(err);
          res.status(503).send(err);
      } else {
...
exports.studentuserCreate = (req, res, next) => {
  req.body.active = true;
  studentModel.create(req.body, (err, r_user) => {
      if(err) {
          console.log(err);
          res.status(503).send(err);
      } else {
